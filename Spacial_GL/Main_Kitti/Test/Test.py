...
ckpt_file = '../Kitti_GL_lam0.5_30.pth'
...
for folder in folderList:
    ...
    for i in range(0,len(frames)-11,10):
        ...
        #ax.imshow(original)
        #ax.set_title("Ground Truth Frame - "+str(i+FRA+1))
        #ax.set_axis_off()
        #ax = plt.subplot(2,1,2)
        #ax.imshow(img_back)
        #ax.set_title("Predicted Frame - "+str(i+FRA+1))
        #ax.set_axis_off()
        #plt.tight_layout()
        #plt.savefig("Results/"+str(i+FRA+1)+".png")
        # if not os.path.exists(os.path.join("Results0", folder, str(i+10+1))):
        #     os.makedirs(os.path.join("Results0", folder, str(i+10+1)))
        # plt.imsave(os.path.join("Results0", folder, str(i+10+1),'GTFrame-%04d' % (i+10+1)+'.png'), original)
        # plt.imsave(os.path.join("Results0", folder, str(i+10+1),'PDFrame-%04d' % (i+10+1)+'.png'), img_back)
        # plt.close()
        ...
        if (i==0):
            with open('measures.txt', 'w') as f:
                f.write('mse: '+ str(np.mean(np.array(mse))) + '\n')
                f.write('psnr: '+str(np.mean(np.array(psnr))) + '\n')
                f.write('ssim: '+str(np.mean(np.array(ssim))) + '\n\n' + str(folder) + ' ' +str(i))
